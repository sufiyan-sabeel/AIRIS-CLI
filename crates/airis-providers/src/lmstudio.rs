//! LM Studio / local GGUF provider.
//!
//! LM Studio exposes an OpenAI-compatible API on localhost:1234 by default.
//! This provider reuses the OpenAI-compatible request/response format but
//! is tailored for local model serving with no API key requirement and
//! different default model behavior.
...
///
/// LM Studio runs models locally and exposes an OpenAI-compatible endpoint.
/// No API key is needed, and models are whatever GGUF files are loaded.
...
impl LMStudioProvider {
    /// Create a new LM Studio local provider.
    pub fn new(
        client: reqwest::Client,
        base_url: String,
        models: Vec<String>,
    ...
    /// Convert AIRIS messages to OpenAI-format messages (same as OpenAI compat).
    ...
    /// Parse an OpenAI-compatible response into a Message.
    fn parse_chat_response(&self, data: &serde_json::Value) -> AirisResult<Message> {
        ...
        let msg_obj = choice.get("message");
...
impl Provider for LMStudioProvider {
    ...
    async fn list_models(&self) -> AirisResult<Vec<ModelConfig>> {
        if !self.models.is_empty() {
            return Ok(self
                ...
                .map(|m| {
                    ModelConfig {
                        id: ModelId(m.clone()),
                        provider: ProviderId("lmstudio".into()),
                        display_name: m.clone(),
                        capabilities: ModelCapabilities {
                            supports_streaming: true,
                            supports_tools: true,
                            supports_vision: m.contains("vision") || m.contains("llava"),
        ...
        // Try to fetch from LM Studio API
        let url = self.models_url();
        match self.client.get(&url).send().await {
            Ok(resp) => {
                let data: serde_json::Value = resp.json().await.map_err(|e| {
                    AirisError::Http(format!("Failed to parse LM Studio models: {}", e))
                })?;
            ...
            Err(e) => {
                tracing::warn!("Failed to fetch models from LM Studio: {}", e);
                Ok(vec![ModelConfig {
                    id: ModelId("local-model".into()),
                    provider: ProviderId("lmstudio".into()),
            ...
            match es.next().await {
                ...
                Some(Ok(reqwest_eventsource::Event::Message(msg))) => {
                    ...
                    match serde_json::from_str::<serde_json::Value>(&msg.data) {
                        ...
                        Err(e) => {
                            tracing::warn!("Failed to parse LM Studio SSE chunk: {}", e);
                        }
                    }
                }
                Some(Err(e)) => {
