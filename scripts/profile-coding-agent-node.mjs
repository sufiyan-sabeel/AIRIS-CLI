...
const agentDirEnvName = "AIRIS_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "AIRIS_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific AIRIS_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force AIRIS_OFFLINE=1 / AIRIS_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.AIRIS_OFFLINE = "1";
		env.AIRIS_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "airis-startup-benchmark-")) : undefined;
