...
function serializeImageModalities(modalities: readonly ("text" | "image")[]): string {
	return `[${modalities.map((modality) => JSON.stringify(modality)).join(", ")}]`;
}
...
function generateImageModelsFile(models: ImagesModel<"openrouter-images">[]): string {
	const modelEntries = models
		...
		.map(
			...
			input: ${serializeImageModalities(model.input)},
			output: ${serializeImageModalities(model.output)},
