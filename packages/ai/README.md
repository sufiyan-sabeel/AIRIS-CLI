...
```bash
npm install @sufiyan-sabeel/airis-ai
```

TypeBox exports are re-exported from `@sufiyan-sabeel/airis-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@sufiyan-sabeel/airis-ai';
...
```typescript
import { Type, Tool, StringEnum } from '@sufiyan-sabeel/airis-ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@sufiyan-sabeel/airis-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@sufiyan-sabeel/airis-ai';
...
```typescript
import { getImageModel, generateImages } from '@sufiyan-sabeel/airis-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@sufiyan-sabeel/airis-ai';
...
```typescript
import { getModel, stream } from '@sufiyan-sabeel/airis-ai';
...
```typescript
import { getProviders, getModels, getModel } from '@sufiyan-sabeel/airis-ai';
...
```typescript
import { Model, stream } from '@sufiyan-sabeel/airis-ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are airis thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@sufiyan-sabeel/airis-ai';
...
```typescript
import { getModel, complete, Context } from '@sufiyan-sabeel/airis-ai';
...
```typescript
import { Context, getModel, complete } from '@sufiyan-sabeel/airis-ai';
...
- OAuth login flows are not supported in browser environments. Use the `@sufiyan-sabeel/airis-ai/oauth` entry point in Node.js.
...
| Azure OpenAI | `AZURE_OPENAI_AAIRIS_KEY` + `AZURE_OPENAI_BASE_URL` (e.g. `https://{resource}.openai.azure.com`) or `AZURE_OPENAI_RESOURCE_NAME`. Supports `*.openai.azure.com` and `*.cognitiveservices.azure.com`; root endpoints auto-normalize to `/openai/v1`. Optional: `AZURE_OPENAI_AAIRIS_VERSION` (default `v1`), `AZURE_OPENAI_DEPLOYMENT_NAME_MAP`. |
...
| Vertex AI | `GOOGLE_CLOUD_AAIRIS_KEY` or `GOOGLE_CLOUD_PROJECT` (or `GCLOUD_PROJECT`) + `GOOGLE_CLOUD_LOCATION` + ADC |
...
| Xiaomi MiMo (API billing) | `XIAOMI_AAIRIS_KEY` |
| Xiaomi MiMo Token Plan (China) | `XIAOMI_TOKEN_PLAN_CN_AAIRIS_KEY` |
| Xiaomi MiMo Token Plan (Amsterdam) | `XIAOMI_TOKEN_PLAN_AMS_AAIRIS_KEY` |
| Xiaomi MiMo Token Plan (Singapore) | `XIAOMI_TOKEN_PLAN_SGP_AAIRIS_KEY` |
| GitHub Copilot | `COPILOT_GITHUB_TOKEN` |

When set, the library automatically uses these keys:

```typescript
// Uses OPENAI_AAIRIS_KEY from environment
const model = getModel('openai', 'gpt-4o-mini');
const response = await complete(model, context);
...
```typescript
import { getEnvApiKey } from '@sufiyan-sabeel/airis-ai';

// Check if an API key is set in environment variables
const key = getEnvApiKey('openai');  // checks OPENAI_AAIRIS_KEY
```
...
- **API key**: Set `GOOGLE_CLOUD_AAIRIS_KEY` or pass `apiKey` in the call options.
...
When using ADC, also set `GOOGLE_CLOUD_PROJECT` (or `GCLOUD_PROJECT`) and `GOOGLE_CLOUD_LOCATION`. You can also pass `project`/`location` in the call options. When using `GOOGLE_CLOUD_AAIRIS_KEY`, `project` and `location` are not required.
...
(async () => {
  ...
  const response = await complete(model, {
    ...
    apiKey: process.env.GOOGLE_CLOUD_AAIRIS_KEY,
  });
...
```bash
npx @sufiyan-sabeel/airis-ai login              # interactive provider selection
npx @sufiyan-sabeel/airis-ai login anthropic    # login to specific provider
npx @sufiyan-sabeel/airis-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@sufiyan-sabeel/airis-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@sufiyan-sabeel/airis-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@sufiyan-sabeel/airis-ai/oauth';
...
**Azure OpenAI (Responses)**: Uses the Responses API only. Set `AZURE_OPENAI_AAIRIS_KEY` and either `AZURE_OPENAI_BASE_URL` or `AZURE_OPENAI_RESOURCE_NAME`. `AZURE_OPENAI_BASE_URL` supports both `https://<resource>.openai.azure.com` and `https://<resource>.cognitiveservices.azure.com`; root endpoints are normalized to `.../openai/v1` automatically. Use `AZURE_OPENAI_AAIRIS_VERSION` (defaults to `v1`) to override the API version if needed. Deployment names are treated as model IDs by default, override with `azureDeploymentName` or `AZURE_OPENAI_DEPLOYMENT_NAME_MAP` using comma-separated `model-id=deployment` pairs (for example `gpt-4o-mini=my-deployment,gpt-4o=prod`). Legacy deployment-based URLs are intentionally unsupported.
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@sufiyan-sabeel/airis-ai`
- Add credential detection in `env-aairis-keys.ts` for the new provider
