 ...
 * - ~/.airis/agent/presets.json (global)
 * - <cwd>/.airis/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `airis --preset plan` - start with plan preset
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".airis", "presets.json");
...
export default function presetExtension(airis: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: airis.getThinkingLevel(),
				tools: airis.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			airis.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = airis.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				airis.setActiveTools(validTools);
			}
		}
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.airis/agent/presets.json or .airis/presets.json",
				"warning",
			);
			return;
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await airis.setModel(originalState.model);
				}
				airis.setThinkingLevel(originalState.thinkingLevel);
				airis.setActiveTools(originalState.tools);
			} else {
				airis.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	airis.registerShortcut(Key.ctrlShift("u"), {
	...
	airis.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = airis.getFlag("preset");
	...
	airis.on("turn_start", async () => {
		if (activePresetName) {
			airis.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
