...
export default function (airis: ExtensionAPI) {
	...
	airis.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "airis RPC Demo (new session)" : "airis RPC Demo");
	...
	airis.registerCommand("rpc-input", {
	...
	airis.registerCommand("rpc-editor", {
	...
	airis.registerCommand("rpc-prefill", {
