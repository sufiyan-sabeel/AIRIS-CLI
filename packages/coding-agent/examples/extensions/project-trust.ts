 ...
 *
 *   mkdir -p ~/.airis/agent/extensions
 *   cp packages/coding-agent/examples/extensions/project-trust.ts ~/.airis/agent/extensions/
 *
 * Or:
 *
 *   airis -e packages/coding-agent/examples/extensions/project-trust.ts
 *
 * Try it in a project containing .airis, AGENTS.md/CLAUDE.md, or .agents/skills.
 */

import type { ExtensionAPI, ProjectTrustEventResult } from "@sufiyan-sabeel/airis-cli";
...
export default function (airis: ExtensionAPI) {
	...
	airis.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {
