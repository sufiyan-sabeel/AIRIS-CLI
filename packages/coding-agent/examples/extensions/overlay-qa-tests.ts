 ...
 *
 * Usage: airis --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@sufiyan-sabeel/airis-tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@sufiyan-sabeel/airis-tui";
...
export default function (airis: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like airis-doom would need)
	airis.registerCommand("overlay-animation", {
	...
	airis.registerCommand("overlay-anchors", {
	...
	airis.registerCommand("overlay-margins", {
	...
	airis.registerCommand("overlay-stack", {
	...
	airis.registerCommand("overlay-overflow", {
	...
	airis.registerCommand("overlay-edge", {
	...
	airis.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	airis.registerCommand("overlay-maxheight", {
	...
	airis.registerCommand("overlay-sidepanel", {
	...
	airis.registerCommand("overlay-toggle", {
	...
	airis.registerCommand("overlay-passive", {
	...
	airis.registerCommand("overlay-focus", {
	...
	airis.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/airis-mono/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/airis-mono\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like airis-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (airis-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
