...
async function resolveGitHubRepo(airis: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await airis.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (airis: ExtensionAPI): void {
	airis.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(airis, ctx.cwd);
		if (!resolvedRepo.ok) {
