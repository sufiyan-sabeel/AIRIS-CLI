 ...
 * - ~/.airis/agent/extensions/sandbox.json (global)
 ...
 *
 * Usage:
 * - `airis -e ./sandbox` - sandbox enabled with default/config settings
 * - `airis -e ./sandbox --no-sandbox` - disable sandboxing
 * - `/sandbox` - show current sandbox configuration
 *
 * Setup:
 * 1. Copy sandbox/ directory to ~/.airis/agent/extensions/
 * 2. Run `npm install` in ~/.airis/agent/extensions/sandbox/
 *
...
export default function (airis: ExtensionAPI) {
	...
	airis.on("session_start", async (_event, ctx) => {
		const noSandbox = airis.getFlag("no-sandbox") as boolean;
