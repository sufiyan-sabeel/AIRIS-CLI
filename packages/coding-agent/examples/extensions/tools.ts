...
export default function toolsExtension(airis: ExtensionAPI) {
	// Track enabled tools
	...
	function persistState() {
		airis.appendEntry<ToolsState>("tools-config", {
	...
	function applyTools() {
		airis.setActiveTools(Array.from(enabledTools));
	}

	// Find the last tools-config entry in the current branch
	function restoreFromBranch(ctx: ExtensionContext) {
		allTools = airis.getAllTools();
		...
		if (savedTools) {
			...
			enabledTools = new Set(airis.getActiveTools());
		}
	}

	// Register /tools command
	airis.registerCommand("tools", {
	...
	airis.on("session_tree", async (_event, ctx) => {
		restoreFromBranch(ctx);
	});
}
