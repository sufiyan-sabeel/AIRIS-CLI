...
export default function (airis: ExtensionAPI) {
	airis.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await airis.exec("git", ["status", "--porcelain"]);
		...
		// Stage and commit
		await airis.exec("git", ["add", "-A"]);
		const { code: commitCode } = await airis.exec("git", ["commit", "-m", commitMessage]);
