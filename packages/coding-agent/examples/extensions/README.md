...
Example extensions for airis-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
airis --extension examples/extensions/permission-gate.ts
...
| `send-user-message.ts` | Demonstrates `airis.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs airis theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `airis.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using airis-ai's built-in Anthropic/OpenAI streaming via proxy |
