> airis can create themes. Ask it to build one for your setup.
...
AIRIS loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.airis/agent/themes/*.json`
- Project: `.airis/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `airis.themes` entries in `package.json`
...
On first run, airis detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.airis/agent/themes
vim ~/.airis/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/sufiyan-sabeel/AIRIS-CLI/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, airis reloads it automatically for immediate visual feedback.
...
AIRIS uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, airis falls back to the nearest approximation.
