...
```
~/.airis/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.airis/agent/sessions/`.

AIRIS also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, AIRIS uses the `trash` CLI to avoid permanent deletion.
...
Source on GitHub ([airis-mono](https://github.com/earendil-works/airis-mono)):
- [`packages/coding-agent/src/core/session-manager.ts`](https://github.com/earendil-works/airis-mono/blob/main/packages/coding-agent/src/core/session-manager.ts) - Session entry types and SessionManager
- [`packages/coding-agent/src/core/messages.ts`](https://github.com/earendil-works/airis-mono/blob/main/packages/coding-agent/src/core/messages.ts) - Extended message types (BashExecutionMessage, CustomMessage, etc.)
...
### Base Message Types (from airis-ai)
...
### Extended Message Types (from airis-coding-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if airis-generated (legacy field name)
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `airis.setSessionName()` in extensions.
