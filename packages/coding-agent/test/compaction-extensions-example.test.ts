...
describe("Documentation example", () => {
	it("custom compaction example should type-check correctly", () => {
		...
		const exampleExtension = (airis: ExtensionAPI) => {
			airis.on("session_before_compact", async (event: SessionBeforeCompactEvent, ctx) => {
	...
	it("compact event should have correct fields", () => {
		const checkCompactEvent = (airis: ExtensionAPI) => {
			airis.on("session_compact", async (event: SessionCompactEvent) => {
