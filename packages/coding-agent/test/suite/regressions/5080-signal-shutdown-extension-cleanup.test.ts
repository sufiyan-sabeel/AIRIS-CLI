...
// Regression for https://github.com/sufiyan-sabeel/AIRIS-CLI/issues/5080
//
...
function createSessionManager(options: { sessionFile?: string } = {}): SessionManager {
	return {
		...
		getSessionDir: () => "/tmp/airis-sessions",
		usesDefaultSessionDir: () => true,
	} as unknown as SessionManager;
}

function createTempFile(): string {
	const dir = mkdtempSync(join(tmpdir(), "airis-shutdown-resume-hint-"));
	tempDirs.push(dir);
