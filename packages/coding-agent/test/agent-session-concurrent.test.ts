...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `airis-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(airis) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = airis;
			},
			(airis) => {
		...
		expect(airis).toBeDefined();

		airis!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
