...
Options:
  -l, --local       Install project-locally (.airis/settings.json)
...
Options:
  -l, --local       Remove from project settings (.airis/settings.json)
  -a, --approve     Trust project-local files for this command
  -na, --no-approve Ignore project-local files for this command
...
Update AIRIS and installed packages.

Options:
  --self                  Update AIRIS only
  --extensions            Update installed packages only
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall AIRIS even if the current version is latest

Short forms:
  ${APP_NAME} update                Update AIRIS and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update self             Update AIRIS only (self works as alias to self)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "airis";
			if (sourceIsSelf) {
...
function printSelfUpdateUnavailable(npmCommand?: string[], updatePackageName = PACKAGE_NAME): void {
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of AIRIS executable: ${entrypoint}`);
	}
}
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	...
	try {
		const latestRelease = await getLatestAirisRelease(VERSION);
